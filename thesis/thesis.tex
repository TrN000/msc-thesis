...
\end{minted}
\caption{Turning an equivalence relation into a quotient type using \lstinline|Quot| and \lstinline|Setoid|.}
...
instance : Add ℤ' := ⟨zadd⟩
\end{minted}
\caption{Lifting addition of the carrier type to the quotient type using \lstinline|Quot.lift|.}
...
Qed.
\end{minted}
\caption{Proff that \lstinline|add_pair_Proper| is commutative.}
...
Lean, by its own admission fails at subject reduction.
We show how it fails maybe by the example shown in \cite[Carneiro's Thesis]{carneiro_2019}.
This is not the only example, but many of known examples are built in a similar manner.
We take it as a good representative of
...
\inputminted{lean4}{../src/Src/SetoidHell.lean}

\section{SetoidHell.v}

\inputminted{coq}{../src/SetoidHell.v}
