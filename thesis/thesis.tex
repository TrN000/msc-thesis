...
\chapter{Introduction to Dependent Type Theory for Proof Assistants}\label{chap:introduction_to_dependent_type_theory_for_proof_assistants} % (fold)
...
Setoid hell is often cited as an argument in favor of Lean over Rocq, arguing that the working mathematician will be better enabled to do pragmatic work in Lean, whereas in Rocq they would have to deal with setoids, that is a structure consisting of the underlying type and an equivalence relation, instead of a quotient type, that makes the underlying construction "irrelevant" (irrelevant in the sense that it is not in the way).

The narrative throughline we pick to investigate this problem is to begin with an example of setoids and quotients in both Lean and Rocq, and work our way backwards through definitions to root-cause the distinguishing attributes of both systems that give rise to this phenomenon.
...
\begin{minted}{lean4}[captionpos=b, caption="Turning an equivalence relation into a quotient type using \lstinline|Quot| and \lstinline|Setoid|."]
...
\begin{minted}{lean4}[captionpos=b, caption="Lifting addition of the carrier type to the quotient type using \lstinline|Quot.lift|"]
...
\begin{minted}{coq}[captionpos=b, caption="Definition of the setoid in Rocq"]
...
\begin{minted}[captionpos=b, caption="Creating a proper morphism of addition in Rocq."]
...
\begin{minted}[captionpos=b, caption="Proff that \lstinline|add_pair_Proper| is commutative."]
...
% Go by the following citations

\cite[lean4/src/kernel/quot.h at master (at time of writing)]{lean4_source}
What Lean implements is Quot, quot lift, quot ind, and quot mk.

% what are these, take a look at the adjacent quot.cpp file
...
Lean, by its own admission fails at subject reduction. We show how it fails maybe by \cite[Carneiro's Thesis]{carneiro_2019}
